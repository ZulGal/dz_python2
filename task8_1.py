...
def content_of_dir(namedir: str) -> None:
    data: dict = [{'name': namedir, 'parent': None, 'type': 'dir', 'size': get_size(namedir)}]
    ...
    for dir_path, dir_name, file_name in os.walk(os.getcwd()):
        ...
        data.extend(
            {'name': i, 'parent': dir_path.split('/')[-1], 'type': 'dir', 'size': get_size(i)} for i in dir_name)
    print(f'{data=}')
    ...
    with (
        ...
        csv_write = csv.DictWriter(out_csv, dialect='excel', fieldnames=["name", "parent", "type", "size"])
