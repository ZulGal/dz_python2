...
def save_operations(type_oper: str) -> None:
    global save_list
    save_list.append(type_oper)
...
def withdraw():
    ...
    while sum_operation % SUM_OPERATION != 0 or sum_operation < MIN_WITHDRAW or sum_operation > MAX_WITHDRAW:
        sum_operation = int(input(
            f"Введите сумму снятия (кратно {SUM_OPERATION} не меньше {MIN_WITHDRAW}\
             и не больше {MAX_WITHDRAW} или остатка на счете = {sum}: "))
    sum -= sum_operation
    print(f"Баланс: {sum}")
...
main_instruction = 'Введите команду top (пополнить), withdraw (снять), print(печать операций), exit (выйти):   '
command = ''
...
while command != 'exit':
    ...
    if command == 'top':
        wealth_tax()
        save_operations('wealth_tax')
        top_up()
        save_operations('top_up')
    elif command == 'withdraw':
        wealth_tax()
        save_operations('wealth_tax')
        withdraw()
        save_operations('withdraw')
    elif command == 'print':
        print(save_list)
